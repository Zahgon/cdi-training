...
The application teaches the concepts of the link:https://jakarta.ee/specifications/cdi/3.0/[CDI 3.0] specification, implemented on link:https://spring.io/projects/spring-boot[Spring Boot 3] with Spring MVC and Thymeleaf.
Every CDI concept below is demonstrated through its Spring equivalent (`@Component`/`@Bean` for beans and producers, `@RequestScope`/`@SessionScope` for contexts, `@Qualifier` for qualifiers, Spring AOP for interceptors, `ApplicationEventPublisher`/`@EventListener` for events, and a custom `Scope` implementation for the custom-scope exercise).
...
* link:cdi-introduction-v1.3.pdf[CDI Introduction] +
...
No application server is required: Spring Boot starts an embedded Tomcat.
...
----
mvn clean package
java -jar target/cdi-training-2.1.0-SNAPSHOT.jar
----
...
Import the project as a Maven project and run `at.gepardec.training.cdi.CdiTrainingApplication` as a plain Java application. No server integration is needed.
